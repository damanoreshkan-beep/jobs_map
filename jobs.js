(()=>{
  ...
  const C = (t, p = {}) => Object.assign(D.createElement(t), p);
  ...
  css.textContent = `
    ...
    @media (max-width:900px){
      #jc-panel{
        left:0;right:0;bottom:0;margin:0;width:100%;max-width:none;
    ...
    #jc-bar>i{
      ...
      background:linear-gradient(90deg,#0b4,#0f8)
    }
    #jc-bar>u{
      position:absolute;inset:0;
      background:repeating-linear-gradient(45deg,transparent 0 12px, rgba(255,255,255,.06) 12px 24px)
    }
    #jc-stats{display:flex;justify-content:space-between;gap:10px;margin-top:8px;color:#9cff9c;font-weight:700}
    #jc-grid{
      margin-top:12px;height:160px;position:relative;border-radius:12px;background:#051;
      overflow:hidden;border:1px solid rgba(0,255,120,.15)
    }
  ...
  const chooseArray = j => {
    ...
    if (j && typeof j === 'object') {
      ...
      const firstArr = Object.values(j).find(v => Array.isArray(v));
  ...
  const sLeft = C('div', { textContent: '0 з 0' });
  ...
  loadCard.append(loadTitle, bar, stats, grid);
  ...
  const startGridAnim = () => {
    ...
    const fit = () => {
      ...
      gridCanvas.width = r.width;
      gridCanvas.height = r.height;
    };
    fit();
    ...
    const step = () => {
      ...
      for (const d of dots) {
        ...
        ctx.strokeStyle = `rgba(0,255,140,${0.25 * (1 - phase)})`;
        ctx.lineWidth = 1.5;
        ctx.stroke();
  ...
  const renderPanel = (it, i) => {
    if (!it) {
      ttl.textContent = 'Наведіть на точку…';
      cnt.textContent = items.length ? `0 / ${items.length}` : '';
      metaBox.innerHTML = '';
      return;
    }
    ...
    for (const [k, v] of pairs.slice(0, 80)) {
      ...
      if (/^https?:\/\//.test(v)) {
        dv.append(C('a', { href: v, target: '_blank', textContent: v, style: 'color:#9cff9c;text-decoration:underline' }));
      } else {
        dv.textContent = v;
      }
      kv.append(dk, dv);
  ...
  const orbitPositions = (la, lo, n) => {
    ...
    while (left > 0) {
      ring++;
      const on = Math.min(left, perRing);
      const r = base + step * (ring - 1);
      for (let i = 0; i < on; i++) {
  ...
  const addAllPoints = arr => {
    ...
    const withCoords = arr
      .map((it, i) => ({ it, i, la: +latOf(it), lo: +lonOf(it) }))
      .filter(v => Number.isFinite(v.la) && Number.isFinite(v.lo));

    const groups = new Map();
    for (const r of withCoords) {
      const k = keyCoord(r.la, r.lo);
      if (!groups.has(k)) groups.set(k, []);
      groups.get(k).push(r);
    }

    for (const [, grp] of groups.entries()) {
      const baseLa = grp[0].la;
      const baseLo = grp[0].lo;
      const pos = orbitPositions(baseLa, baseLo, grp.length);
      grp.forEach((g, idx) => {
        const [la, lo] = pos[idx];
  ...
  const startBackAnim = () => {
    ...
    fit();
    S.addEventListener('resize', fit, { passive: true });
    ...
    const step = () => {
      ...
      for (const d of dots) {
        ...
        const ph = d.t / 420;
        const r1 = 8 + 40 * ph;
        const r2 = 18 + 70 * ((ph + .33) % 1);
        const r3 = 28 + 100 * ((ph + .66) % 1);
        const al = .28 * (1 - ph);

        ctx.beginPath();
        ctx.arc(d.x, d.y, r1, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(0,255,140,${al})`;
        ctx.lineWidth = 1;
        ctx.stroke();

        ctx.beginPath();
        ctx.arc(d.x, d.y, r2, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(0,200,120,${al * .8})`;
        ctx.lineWidth = 1;
        ctx.stroke();

        ctx.beginPath();
        ctx.arc(d.x, d.y, r3, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(0,150,100,${al * .6})`;
        ctx.lineWidth = 1;
        ctx.stroke();
