(()=>{
  ...
  const root=C('div',{id:'jc-root'}),anim=C('canvas',{id:'jc-anim','aria-hidden':'true'}),mapWrap=C('div',{id:'jc-map'}),panel=C('div',{id:'jc-panel','aria-live':'polite'}),head=C('div',{id:'jc-head'}),ttl=C('div',{id:'jc-ttl',textContent:'Готуємо дані…'}),cnt=C('div',{id:'jc-cnt',textContent:'0 / 0'}),pro=C('button',{id:'jc-pro',textContent:'PRO $4.99/y',type:'button'}),metaBox=C('div',{id:'jc-meta'});
  head.append(ttl,cnt,pro); panel.append(head,metaBox); root.append(anim,mapWrap,panel); D.body.append(root);
  ...
  const css=C('style'); css.textContent=`
    ...
    #jc-panel{position:absolute;right:16px;bottom:16px;z-index:3;width:min(480px,36vw);max-width:92vw;height:42vh;max-height:78vh;min-height:220px;display:grid;grid-template-rows:auto 1fr;gap:12px;padding:14px 14px 12px;background:linear-gradient(to bottom right,rgba(10,20,10,.58),rgba(8,12,8,.34));border:1px solid rgba(180,255,210,.18);border-right-color:rgba(180,255,210,.28);border-top-color:rgba(180,255,210,.24);box-shadow:0 18px 50px rgba(0,0,0,.55), inset 0 1px 0 rgba(255,255,255,.06);backdrop-filter:blur(14px) saturate(1.18);-webkit-backdrop-filter:blur(14px) saturate(1.18);border-radius:14px}
    ...
    #jc-pro{appearance:none;border:1px solid rgba(0,255,120,.25);background:rgba(0,0,0,.35);color:#b9ffb9;font-weight:800;border-radius:999px;padding:8px 12px}
    ...
    @media (max-width:900px){#jc-panel{left:0;right:0;bottom:0;margin:0;width:100%;max-width:none;height:52vh;max-height:86vh;border-radius:16px 16px 0 0;padding:16px 14px 12px}.kv{grid-template-columns:110px 1fr}#jc-ttl{font-size:17px}}
    @media (max-width:560px){#jc-panel{height:58vh}.kv{grid-template-columns:96px 1fr}}
    #jc-load{position:absolute;inset:0;z-index:4;display:grid;place-items:center;background:radial-gradient(1200px 800px at 50% 50%, rgba(0,40,20,.35), rgba(0,0,0,.85));backdrop-filter:blur(6px) saturate(1.05);-webkit-backdrop-filter:blur(6px) saturate(1.05)}
    #jc-load-card{width:min(720px,90vw);border-radius:16px;padding:18px 18px 14px;background:linear-gradient(to bottom right, rgba(10,30,16,.6), rgba(6,14,10,.36));border:1px solid rgba(0,255,140,.15);box-shadow:0 18px 60px rgba(0,0,0,.6), inset 0 1px 0 rgba(255,255,255,.04)}
  ...
  const loadOverlay=C('div',{id:'jc-load'}),
        loadCard=C('div',{id:'jc-load-card'}),
        loadTitle=C('h3',{id:'jc-load-title',textContent:'Завантаження вакансій…'}),
        bar=C('div',{id:'jc-bar'}),
        barFill=C('i'),
        barSheen=C('u'); // ← fixed: was C('u'}))

  bar.append(barFill,barSheen);

  const stats=C('div',{id:'jc-stats'}),
        sLeft=C('div',{textContent:'0 файлів'}),
        sPct=C('div',{textContent:'0%'});
  stats.append(sLeft,sPct);

  const sub=C('div',{id:'jc-substats'}),
        sMB=C('div',{textContent:'0.00 / 0.00 MB'}),
        sSp=C('div',{textContent:'0.0 MB/s'}),
        sEta=C('div',{textContent:'ETA — ∞'});
  sub.append(sMB,sSp,sEta);

  const grid=C('div',{id:'jc-grid'}),gridCanvas=C('canvas'); grid.append(gridCanvas);
  loadCard.append(loadTitle,bar,stats,sub,grid); loadOverlay.append(loadCard); root.append(loadOverlay);
  ...
  const startBackAnim=()=>{
    ...
    const dots=[...Array(N)].map(()=>({x:Math.random()*anim.width,y:Math.random()*anim.height,vx:(Math.random()-.5)*.25,vy:(Math.random()-.5)*.25,t:Math.random()*400|0}));
    const step=()=>{
  ...
  const radiusForZoom=z=>clamp(2 + (z-2)*(8/18), 2, 10); // z∈[2..20] → r∈[2..10]
  const stylesForZoom=z=>{
    const r=radiusForZoom(z);
    return {
      normal:{radius:r,weight:1,color:'#00ff78',fillColor:'#00aa55',fillOpacity:.75},
      active:{radius:r+3,weight:2,color:'#7bffb7',fillColor:'#00dd77',fillOpacity:.9}
    };
  };
  ...
  const highlight=idx=>{
    const z=map?.getZoom()??12; const st=stylesForZoom(z);
    if(current!=null&&markers[current]) markers[current].setStyle(st.normal);
    current=idx;
    if(markers[idx]) markers[idx].setStyle(st.active);
  };

  const updateMarkerSizes=()=>{
    if(!map) return;
    const z=map.getZoom(), st=stylesForZoom(z);
    for(let i=0;i<markers.length;i++){
      const m=markers[i]; if(!m) continue;
      const isActive=i===current;
      m.setStyle(isActive?st.active:st.normal);
    }
  };
  ...
  const renderPanel=(it,i)=>{
    ...
    ttl.textContent=pickTitle(it); cnt.textContent=`${i+1} / ${items.length}`;
  ...
  const orbitPositions=(la,lo,n)=>{
    ...
    while(left>0){
      ring++;
      const on=Math.min(left,perRing), r=base+step*(ring-1);
      for(let i=0;i<on;i++){
  ...
  // Data loading with real MB progress
  const getPicks=async()=>{ const list=await fetch('./jobs/chunks-list.json',{cache:'no-store'}).then(r=>{if(!r.ok) throw new Error('chunks-list.json not found'); return r.json()}).then(j=>Array.isArray(j)?j:chooseArray(j)).catch(e=>{log('Error list:',e); return []}); log('Chunk URLs:',list); return list };

  const fetchOne=async u=>{
    let szKnown=false,recvd=0;
    try{
      const resp=await fetch('./'+u,{cache:'no-store'});
      if(!resp.ok) throw new Error('HTTP '+resp.status);
      const cl=+resp.headers.get('content-length')||0; if(cl>0){ bytesTotal+=cl; szKnown=true; setProgress() }
      const reader=resp.body?.getReader?.(); const chunks=[];
      if(reader){ for(;;){ const {done,value}=await reader.read(); if(done) break; recvd+=value.byteLength; bytesDone+=value.byteLength; chunks.push(value); setProgress() } }
      else{ const ab=await resp.arrayBuffer(); recvd=ab.byteLength; bytesDone+=recvd; chunks.push(new Uint8Array(ab)); if(!szKnown){ bytesTotal+=recvd; setProgress() } }
      const full=new Blob(chunks,{type:'application/json'}); const j=JSON.parse(await full.text()); const arr=chooseArray(j); if(arr?.length) items.push(...arr);
    }catch(e){ log('fail',u,e?.message||e) }
    finally{ filesDone++; setProgress() }
  };

  const topIntro=()=>{ metaBox.innerHTML=''; const kv=C('div',{className:'kv'}); [['hint','Наведіть на точку на карті'],['loaded','Очікуємо завантаження…'],['pro','PRO відкриє фільтри, пошук, експорт']].forEach(([k,v])=>{ kv.append(C('div',{className:'k',textContent:k}),C('div',{className:'v',textContent:v})) }); metaBox.append(kv) };
  ...
  const load=async()=>{
    ...
    log('Total items:',items.length); loadTitle.textContent=items.length?`Готово: ${items.length} вакансій`:'Даних не знайдено';
    const ok=await setupMap(); if(ok&&items.length){ addAllPoints(items); renderPanel(null,-1); cnt.textContent=`0 / ${items.length}` } else ttl.textContent='Не вдалося відрендерити карту';
